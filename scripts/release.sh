  ...
  case "${ver_raw}" in
    patch|minor|major)
      ...
      if [[ -z "${latest:-}" ]]; then
        prefix=""
        case "${bump}" in
          patch) ver="${prefix}0.0.1" ;;
          minor) ver="${prefix}0.1.0" ;;
          major) ver="${prefix}1.0.0" ;;
        esac
        ...
        if [[ "${t}" =~ ^([0-9]+)\.([0-9]+)\.([0-9]+) ]]; then
          ...
          case "${bump}" in
            patch)
              ver="${prefix}${major}.${minor}.$((10#${patch} + 1))"
              echo "Bumped patch: ${latest} -> ${ver}" >&2
              ;;
            minor)
              ver="${prefix}${major}.$((10#${minor} + 1)).0"
              echo "Bumped minor: ${latest} -> ${ver}" >&2
              ;;
            major)
              ver="${prefix}$((10#${major} + 1)).0.0"
